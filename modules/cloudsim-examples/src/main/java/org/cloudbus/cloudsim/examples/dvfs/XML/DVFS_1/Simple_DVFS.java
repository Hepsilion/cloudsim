package org.cloudbus.cloudsim.examples.dvfs.XML.DVFS_1;
...
public class Simple_DVFS {
	...
	public static void main(String[] args) {
		...
		try {
			...
			ConfSimu = new SimulationXMLParse("E:/Workspace/Github/cloudsim/modules/cloudsim-examples/src/main/java/org/cloudbus/cloudsim/examples/dvfs/XML/DVFS_1/Experience.xml");
	...
	private static List<Vm> createVms(int userId, int nb_vm, int IdShift,
		...
		for (int i = no_cur_vm; i < (offset + nb_vm); i++) {
			...
			vms.add(new Vm((IdShift + i), userId, mips, pesNumber, ram, bw,
	...
	private static PowerDatacenter createDatacenter(String name) throws Exception {
		...
		double time_zone = tmp_dc.getTimezone(); // 10.0; // time zone this
													// resource located
		double cost = tmp_dc.getCost(); // 3.0; // the cost of using processing
										// in this resource
		double costPerMem = tmp_dc.getCostPerMem(); // 0.05; // the cost of
													// using memory in this
													// resource
		double costPerStorage = tmp_dc.getCostPerStorage(); // 0.001; // the
															// cost of using
															// storage in this
															// resource
		double costPerBw = tmp_dc.getCostPerbW(); // 0.0; // the cost of using
													// bw in this resource
		...
		try {
			powerDatacenter = new PowerDatacenter(
					...
					new LinkedList<Storage>(), 0.01); // fix to 0.1 as the Dvfs Sampling Rate in the Linux Kernel
		} catch (Exception e) {
			e.printStackTrace();
		}
