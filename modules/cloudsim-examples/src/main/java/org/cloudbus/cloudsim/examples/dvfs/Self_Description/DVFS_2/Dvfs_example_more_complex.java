package org.cloudbus.cloudsim.examples.dvfs.Self_Description.DVFS_2;
...
public class Dvfs_example_more_complex {
	...
	public static void main(String[] args) {
		...
		try {
			...
			List<Cloudlet> newList = vect_dcbroker.get(0)
			...
			Log.printLine(String.format("Total simulation time: %.2f sec",
					lastClock));
			Log.printLine(String.format("Power Sum :  %.8f W",
					datacenter.getPower()));
			Log.printLine(String.format("Power Average : %.8f W",
					datacenter.getPower() / (lastClock * 100)));
			Log.printLine(String.format("Energy consumption: %.8f Wh",
					(datacenter.getPower() / (lastClock * 100))
							* (lastClock * 100 / 3600)));
	 ...
	 */
	private static List<Cloudlet> createCloudletList(int userId,
			int nb_cloudlet, int IdShift, int type_cl) {
		...
		for (int i = no_cur_cloudlet; i < (offset + nb_cloudlet); i++) {
			...
			Cloudlet cloudlet = new Cloudlet((IdShift + i), length[type_cl],
			...
			list.add(cloudlet);
			no_cur_cloudlet++;
			System.out.println("Cloudlet created // No de Cloudlet =  "
					+ no_cur_cloudlet + "  //  Cloudlet List Size = "
					+ list.size());
		}
		return list;
	}
	...
	private static List<Vm> createVms(int userId, int nb_vm, int IdShift,
			int type_vm) {
		...
		for (int i = no_cur_vm; i < (offset + nb_vm); i++) {
			vms.add(new Vm((IdShift + i), userId, mips[type_vm], pesNumber,
					ram, bw, size, vmm, new CloudletSchedulerSpaceShared()));
			no_cur_vm++;
			System.out.println("VM created  // No VM =  " + no_cur_vm
					+ "  //  List Vm size = " + vms.size()
					+ "  // ID Shift+1 = " + (IdShift + i) + "id vm = "
					+ vms.get(0).getId());
		}
	...
	private static PowerDatacenter createDatacenter(String name)
		...
		ArrayList<Double> freqs = new ArrayList<>(); // frequencies available by
														// the CPU
		freqs.add(59.925);// frequencies are defined in % , it make free to use
							// Host MIPS like we want.
		freqs.add(69.93); // frequencies must be in increase order !
		freqs.add(79.89);
		freqs.add(89.89);
		freqs.add(100.0);

		HashMap<Integer, String> govs = new HashMap<Integer, String>(); // Define
																		// wich
																		// governor
																		// is
																		// used
		...
		for (int i = 0; i < hostsNumber; i++) {
			...
			for (int pe = 0; pe < nb_pe; pe++) {
				peList.add(new Pe(pe, new PeProvisionerSimple(mips), freqs,
						govs.get(pe), ConfigDvfs));
			}
			hostList.add(new PowerHost(i, new RamProvisionerSimple(ram),
					new BwProvisionerSimple(bw), storage, peList,
					new VmSchedulerTimeShared(peList),
					new PowerModelSpecPower_BAZAR(peList), false, enableDVFS
		...
		try {
			powerDatacenter = new PowerDatacenter(
					...
					new LinkedList<Storage>(), 0.01); // fix to 0.1 as the Dvfs
														// Sampling Rate in the
														// Linux Kernel
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	private static PowerDatacenterBroker createBrokerWithEvent(String name,
			EventPostBroker evtp) {
	...
	/**
	 * This GlobalBroker class is used to create successively broker that
	 * launch one or multiple VM. Indeed, a broker can "contain" one or more
	 * VM. One (and only one) cloudlet is attached to one VM. The reasone is
	 * if we use only one VM, this has to have to the same size as the Host
	 * to be able to generate 100% cpu load.
	 * 
	 * In our simulation we want to generate a pre-defined scenario, with
	 * multiple predefine phase. It's more convinient to create different
	 * type of VM and different type of Cloudlet. By mixing these different
	 * type of VM/Cloudlet we can easaly manage the CPU load of the
	 * simulation. The size of the VM define the grain with we are able to
	 * increase/decrease the CPU load, and the Cloudlet length determine the
	 * duration of its execution, depend on which type of VM it is launched.
	 * 
	 * 
	 * DVFS ----
	 * 
	 * This scenario is defined at Maximum frequency. So all broker is set
	 * to have the good CPU load curve at Maximum frequency. When the CPU is
	 * decrease, the execution of VM/CloudLet could be slowed and the
	 * sequence of brokers must take into account this slowdown. If we
	 * create broker with STATIC delay start, it could be works at Maximum
	 * frequency, but not at all for another (lower) frequency.
	 * 
	 * So, the launching of a broker could be create dynamically at the end
	 * of another broker. (Event Post Broker)
	 */
	public static class GlobalBroker extends SimEntity {
		...
		private List<PowerDatacenterBroker> created_brokers = Collections
				.synchronizedList(new ArrayList<PowerDatacenterBroker>());
		...
		public void processEvent(SimEvent ev) {
			...
			case STEP_1:
				...
				if (no_broker == 21)
					setBroker(createBrokerWithEvent("Broker_" + no_broker,
							new EventPostBroker(getId(), STEP_2)));
				else
					setBroker(createBroker("Broker_" + no_broker));

				System.out.println("Add a Vm and a Cloudlet... IdBroker="
						+ getBroker().getId());

				vmList_ = createVms(getBroker().getId(), 1, (no_broker * 100),
						type_vm);
				UpdateVmList(vmList_);
				cloudletList_ = createCloudletList(getBroker().getId(), 1,
						(no_broker * 100), type_cl);
			...
			case STEP_2:
				...
				setBroker(createBrokerWithEvent("Broker_" + no_broker,
						new EventPostBroker(getId(), STEP_3)));

				System.out.println("Add a Vm and a Cloudlet... IdBroker="
						+ getBroker().getId());

				vmList_ = createVms(getBroker().getId(), 10, (no_broker * 100),
						type_vm);
				UpdateVmList(vmList_);
				cloudletList_ = createCloudletList(getBroker().getId(), 10,
						(no_broker * 100), type_cl);
				UpdateCloudletList(cloudletList_);
			...
			case STEP_3:
				...
				setBroker(createBrokerWithEvent("Broker_" + no_broker,
						new EventPostBroker(getId(), STEP_4)));
				System.out.println("Add a Vm and a Cloudlet... IdBroker="
						+ getBroker().getId());

				vmList_ = createVms(getBroker().getId(), 16, (no_broker * 100),
						type_vm);
				UpdateVmList(vmList_);
				cloudletList_ = createCloudletList(getBroker().getId(), 16,
						(no_broker * 100), type_cl);
				UpdateCloudletList(cloudletList_);
