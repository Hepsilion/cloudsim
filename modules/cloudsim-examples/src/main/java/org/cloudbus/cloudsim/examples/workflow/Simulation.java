package org.cloudbus.cloudsim.examples.workflow;
...
import org.cloudbus.cloudsim.workflow.Policy;
import org.cloudbus.cloudsim.workflow.Properties;
import org.cloudbus.cloudsim.workflow.SeedGenerator;
import org.cloudbus.cloudsim.workflow.VMOffers;
import org.cloudbus.cloudsim.workflow.WorkflowDatacenter;
import org.cloudbus.cloudsim.workflow.WorkflowEngine;

/**
 * This class contains the main method for execution of the simulation. 
 * Here, simulation parameters are defined. 
 * Parameters that are dynamic are read from the properties file, whereas other parameters are hardcoded.
 * 
 * Decision on what should be configurable and what is hardcoded was somehow arbitrary. 
 * Therefore, if you think that some of the hardcoded values should be customizable, it can be added as a Property. 
 * In the Property code there is comments on how to add new properties to the experiment.
 */
public class Simulation {
	...
	public static void main(String[] args) {
		...
		for (int round = 1; round <= rounds; round++) {
			runSimulationRound(round);System.out.println("Round "+round);
		}
	}

	/**
	 * One round of the simulation is executed by this method. Output is printed to the log.
	 */
	...
	private static WorkflowDatacenter createDatacenter(String name, long seed) throws Exception {
		...
		for (int i = 0; i < hosts; i++) {
			...
			// hostList.add(new PowerHost(i,new RamProvisionerSimple(ram),new
			// BwProvisionerSimple(1000000),
			// storage,peList,new VmSchedulerTimeShared(peList), new
			// PowerModelSpecPower_REIMS(peList) , false));
		}
		...
		/*
		 * PowerDatacenter powerDatacenter = new PowerDatacenter( name,
		 * characteristics, // new PowerVmAllocationPolicySimple(hostList), //
		 * new PowerVmAllocationPolicySimpleWattPerMipsMetric(hostList), //new
		 * PowerVmAllocationPolicyDVFSMinimumUsedHost(hostList), new
		 * PowerVmAllocationPolicySimple(hostList), new LinkedList<Storage>(),
		 * 0.1);
		 * 
