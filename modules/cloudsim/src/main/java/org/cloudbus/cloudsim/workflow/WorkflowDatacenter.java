...
/**
 * This class extends datacenter to support simulation of workflow applications
 * including internal data transfer delays if data has to be moved for task running in other VM.
 */
public class WorkflowDatacenter extends Datacenter {
	...
	// It affects accuracy of the simulation
	...
	public WorkflowDatacenter(String name,
			...
			VmAllocationPolicy vmAllocationPolicy, double bandwidth,
			double latency, int basicCpuUnit, long averageCreationDelay,
			VMOffers vmOffers, long seed) throws Exception {
		...
		if (this.cohostedLatency < QUANTUM)
			cohostedLatency = 0.0;
	...
	protected void processOtherEvent(SimEvent ev) {
		if (ev == null) {
			...
			case UPDATE_NETWORK:
				updateNetwork();
				break;
			case TRANSFER_DATA_ITEM:
				processTransferDataItem(ev);
				break;
			case DATA_ITEM_AVAILABLE:
				processDataItemAvailable(ev);
				break;
	...
	protected void processVmCreate(SimEvent ev, boolean ack) {
		...
		/*
		 * send the ack to ther broker with a random delay, to model delay in VM
		 * boot time (we are charged from the moment we request the Vm, but it
		 * takes a while for it to be usable)
		 */
	...
	protected void processVmDestroy(SimEvent ev, boolean ack) {
		...
		Log.printLine(CloudSim.clock() + ": VM #" + vm.getId()
				+ " destroyed. Total usage time: " + useInHours
				+ " h. Cost = $" + price + " cents/hour.");
	...
	protected void processCloudletSubmit(SimEvent ev, boolean ack) {
		...
		try {
			...
			if (cl.isFinished()) {
				Log.printLine("WARNING: completed cloudlet resubmited. Submission ignored. Cloudlet id:"
						+ cl.getCloudletId());
				return;
			}
			...
			if (estimatedFinishTime < QUANTUM)
				estimatedFinishTime = QUANTUM;
	...
	private void updateNetwork() {
		...
		if (smallerEvent != Double.POSITIVE_INFINITY) {
			if (smallerEvent - CloudSim.clock() < QUANTUM)
				smallerEvent = CloudSim.clock() + QUANTUM;
	...
	private void processDataItemAvailable(SimEvent ev) {
		// send a message to the broker saying that the data item is available
	...
	private void addTransmission(DataItem data, int destinationId,
			int sourceId, boolean cohosted) {
		updateNetwork();

		Transmission transmission = new Transmission(data, sourceId,
				destinationId);
		if (cohosted) {
			// vms are in the same host; Just add a small latency
			schedule(getId(), cohostedLatency, DATA_ITEM_AVAILABLE, transmission);
		} else {
			double delay = getChannel(sourceId, destinationId).addTransmission(
					transmission);
			if (delay >= QUANTUM) {// schedules a completion event
				...
				getChannel(sourceId, destinationId).removeTransmission(
						transmission);
	...
	private long updateVmUsage(long startTime, double price) {
		 ...
		 * simulation purposes. Therefore, we reduce it to minutes discarding
		 * the remainders. Thus, 64 seconds counts as 1 minute. Considering that
		 * the final precision is one hour, this does not induce significant
		 * inaccuracy.
		 */
	...
	public void printSummary() {
		...
		Log.printLine("Total VM Cost: " + df.format(cost));
