...
/**
 * This class handles the whole process of workflow execution, including:
 * parsing xml file, defining number, types, and start times of vms, and scheduling, dispatching, and management of DAG tasks.
 */
public class WorkflowEngine extends SimEntity {
	...
	public void processEvent(SimEvent ev) {
		if (ev == null) {
			...
			case CloudSimTags.CLOUDLET_CANCEL:
				...
				break;
			case WorkflowDatacenter.DATA_ITEM_AVAILABLE:
				processDataItemAvailable(ev);
				break;
			case DELAY_START_EVENT:
				processStartDelayEvent();
				break;
		...
		if (task.hasReplicas()) {
			for (Task replica : task.getReplicas()) {
				...
				if (replica.getCloudlet().getCloudletStatus() == Cloudlet.INEXEC) {
					...
					try {
						replica.getCloudlet().setCloudletStatus(Cloudlet.CANCELED);
					} catch (Exception e) {
					}
				}
			}
	...
	private void processDataItemAvailable(SimEvent ev) {
		...
		Log.printLine(CloudSim.clock() + ": DataItem #"
				+ tr.getDataItem().getId() + " is now available at VM #"
				+ tr.getDestinationId());
		removeOngoingTransmission(tr.getSourceId(), tr.getDestinationId(), tr
				.getDataItem().getId());
	...
	private void moveData(Task task) {
		...
		for (DataItem data : task.getOutput()) {
			for (int destVm : dataRequiredLocation.get(data.getId())) {
				...
				// if origin != destiny and data is not in destiny, trigger transfer
				if (!data.isAvailableAt(destVm)) {
	...
	private void dispatchTasks() {
		...
		for (int vmId : schedulingTable.keySet()) {
			if (freeVmList.get(vmId) == true) {// this vm is available
				...
				if (schedulingTable.get(vmId).isEmpty()) {// no more cloudlets
															// in this vm
					...
					if (task != null) {
						...
						if (dataDependenciesMet) {// required data is available
													// in the VM
							Log.printLine(CloudSim.clock() + ": Task "
									+ task.getId() + " dispatched to VM#"
									+ task.getVmId());
							schedulingTable.get(vmId).remove(0);// remove task
																// from the
																// scheduling
																// table
							freeVmList.put(vmId, false); // vm is busy now
							cloudletTaskMap.put(task.getCloudlet(), task);
							sendNow(datacenterId, CloudSimTags.CLOUDLET_SUBMIT,
									task.getCloudlet());
	...
	public void printExecutionSummary() {
		...
		for (Cloudlet cloudlet : getCloudletReceivedList()) {
			...
			if (cloudlet.getCloudletStatus() == Cloudlet.SUCCESS) {
				 ...
				 * fw.write(cloudlet.getCloudletId() + indent +
				 * dft.format(cloudlet.getSubmissionTime()) + indent +
				 * dft.format(executionTime) + indent + indent +
				 * dft.format(cloudlet.getFinishTime()));
				 * 
	...
	public boolean hasPendingTransmissions(int vmId) {
		...
		while (iter.hasNext()) {
			...
			if (tr.getOriginId() == vmId || tr.getDestId() == vmId)
				return true;
		}
		return false;
	}
	...
	public void computeEnergy() {
		...
		Boolean optimized = Boolean.parseBoolean(Properties.OPTIMIZE
				.getProperty());
		if (optimized) {
		...
		for (Task t : policy.tasks) {
			...
			double length = t.getCloudlet().getFinishTime()
					- t.getCloudlet().getExecStartTime();
			s += dft.format(length) + "\t";
			tmpEner = ener.getTaskEnergyConsumed(t, endTime,
					policy.optimizeScheduling, policy.modedvfs);
			sumEner += tmpEner;
