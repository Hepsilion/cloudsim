...
/**
 * This class encapsulates a DAG Task. 
 * The task contains the actual Cloudlet to be executed, information about its dependencies (data and execution), and scheduling information. 
 */
public class Task {
	...
	protected int optIndexFreq;

	double optExecTime;
	protected double BasicExecTime;
	...
	public Task(Cloudlet cl, int ownerId, double deadlineDag_) {
		this.cloudlet = cl;
		if (cl != null)
			this.cloudlet.setUserId(ownerId);
	...
	public void computeEarlieststart() {
		...
		for (Task tpred : getParents()) {
			// Log.printLine("tache " + this.getId() + " pred Estart : " +
			// tpred.getEarliestStart()+"  , tpred execTime : "+tpred.getOptExecTime());
	...
	public void computeLatestEnd(double deadline) {
		...
		if (!getChildren().isEmpty()) {
			...
			for (Task tsucc : getChildren()) {
				...
				// Log.printLine("tache " + this.getId() + " succ Lend : " +
				// tsucc.getLatestEnd()+"  , tsucc execTime : "+tsucc.getOptExecTime()
				// + " tmpLastEnd = " + tmptime);
	...
	public void computeSlackTime() {
		...
		/*
		 * if(critical) ratioSlackExec=0; else
		 */
		if (!critical)
	...
	/**
	 * Communicates to the Task that its Cloudlet completed. It means that the
	 * dataItem is now available in the VM where it run.
	 *
	 */
